   ...
   <div id="act1-p1-div" class="divide collapse center-text">
      ...
      <div class="fs-18px fb-500">
         S<sub>j</sub> for column 1 = <span style="display:inline-block;">
            $$ \\frac{h_y}{3}(Z_{11} + 4Z_{21} + 2Z_{31} + ... + 2Z_{(2n-1)1} + 4Z_{(2n)1} + Z_{(2n+1)1}) $$
         </span> <br>where j&rarr;0 to 2n+1
      </div>
      <br>
      <div id="tb-box2">
...
function internal_calculations_2() {
    ...
    for (let j = 0; j < raw_data[0].length; j++) {
        s.push(0);
        for (let i = 0; i < (raw_data.length - 2); i += 2) {
            s[j] += raw_data[i][j] + 4 * raw_data[i + 1][j] + raw_data[i + 2][j];
        }
        s[j] = (hy / 3) * s[j];
    }
...
function load_I_div() {
    ...
    btn && btn.remove();
    I = 0;
    for (let j = 0; j < (raw_data.length - 2); j += 2) {
        I += s[j] + 4 * s[j + 1] + s[j + 2];
    }
    I = (hx / 3) * I;
    div.innerHTML += `
      ...
      <div id="I-div">
         <div class="row justify-content-center" style="align-items:center;">
            <div class="col-md-7">
               $$ I = \\frac{h_x}{3}(S_1 + 4S_2 + 2S_3 + ... + 2S_{2n-1}  + 4S_{2n} + S_{2n+1}) = $$
            </div>
            <div class="col-md-4">
...
function verify_I() {
    ...
    div.innerHTML = `
   <div>
   $$ I = \\frac{hx}{2}(S_1 + 2S_2 + 2S_3 + ... + 2S_{2n} + S_{2n+1}) = ${parseFloat(I.toFixed(3))} $$
   </div>
   <br>
